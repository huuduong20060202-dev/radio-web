...
const globe = Globe()(globeContainer)
    ...
    .polygonLabel(({ properties: d }) => `
        <div style="
            ...
            color: white;
        ">
            <b style="color:#64ffda;font-size:14px;">${d.ADMIN}</b>
            <br>
    ...
    // FIX: bỏ fetchCountryAndRadio thủ công — selectCountry đã gọi rồi
...
document.getElementById('tourismBtn').addEventListener('click', () => {
    ...
    const places = currentCountryData.tourism.map(place => `
        <div class="place-card" onclick="googleSearch('${place}')">
            <div class="card-content">
                <h3>✈️ ${place}</h3>
                <p>Nhấp để khám phá trên Google</p>
            </div>
        </div>
    `).join('');
...
// FIX: gán listener 1 lần duy nhất ở ngoài renderSuggestions
// tránh bị gán lặp mỗi lần gõ phím → gọi searchCountry nhiều lần
...
function renderSuggestions() {
    ...
    const matched = allCountriesList
    ...
    suggestionsBox.innerHTML = matched.map(country => `
        <div class="suggest-item" data-country="${country}">
            🌍 ${country}
        </div>
    `).join('');
...
// =======================================================================
// TIMEZONE HELPERS  ← FIX múi giờ
// =======================================================================

// Parse "UTC+07:00" / "UTC-05:30" → milliseconds offset
// Intl.DateTimeFormat không nhận format này nên phải tự parse
...
// KHÔNG cộng thêm getTimezoneOffset() (đó là offset máy local, không liên quan)
...
function fetchCountryAndRadio(code) {
    fetch(`https://restcountries.com/v3.1/alpha/${code}`)
        ...
        .then(async data => {
            ...
            document.getElementById('rpCapital').innerText     = country.capital?.[0] || 'Không rõ';
            ...
            if (country.timezones?.length) {
                ...
                updateClock(); // hiển thị ngay, không chờ 1s
...
async function loadCultureData(baseName, countryCode) {
    try {
        ...
        if (searchData.query.search.length) {
            exactName = searchData.query.search[0].title;
        }
    ...
    stationList.innerHTML = currentStations.map((station, index) => `
        <div class="station-item ${index === currentStationIndex ? 'active' : ''}"
            ...
            <div class="station-meta">
                👍 ${station.votes || 0} • ${station.language || 'Unknown'}
            </div>
        </div>
    `).join('');
