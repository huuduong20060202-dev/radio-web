...
(function initHamburger() {
    ...
    // 2. Tạo nút mở right panel (chỉ hiện trên mobile)
    const rpToggle = document.createElement('button');
    rpToggle.id = 'rightPanelToggle';
    rpToggle.setAttribute('aria-label', 'Thông tin quốc gia');
    rpToggle.innerHTML = '🌍';
    rpToggle.title = 'Thông tin quốc gia';
    document.body.appendChild(rpToggle);

    // 3. Tạo overlay backdrop
    ...
    function openRight() {
        ...
        rightPanel.classList.remove('hidden'); // giữ tương thích với logic cũ
        overlay.classList.add('visible');
        rpToggle.innerHTML = '✕';
    }
    function closeRight() {
    ...
    hamburger.addEventListener('click', () => {
        if (leftPanel.classList.contains('panel-open')) {
            closeLeft();
        } else {
            // Đóng right nếu đang mở
            if (rightPanel.classList.contains('panel-open')) closeRight();
            openLeft();
        }
    });
    ...
    rpToggle.addEventListener('click', () => {
        if (rightPanel.classList.contains('panel-open')) {
            ...
            if (leftPanel.classList.contains('panel-open')) closeLeft();
            openRight();
        }
    });
    ...
    // --- Patch nút close của right panel gốc ---
    const closeRightBtn = document.getElementById('closeRightBtn');
    if (closeRightBtn) {
        const originalClose = closeRightBtn.onclick;
    ...
    // --- Khi quốc gia được chọn → tự mở right panel trên mobile ---
    const observer = new MutationObserver(() => {
        if (!isMobile()) return;
        if (!rightPanel.classList.contains('hidden') ||
             rightPanel.classList.contains('panel-open')) {
            // right panel vừa được app.js mở → chuyển sang panel-open
            if (!rightPanel.classList.contains('panel-open')) {
                openRight();
                closeLeft();
            }
    ...
    observer.observe(rightPanel, { attributeFilter: ['class'] });
